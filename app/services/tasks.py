...
celery_app.conf.update(
    task_routes={
        ...
        'app.services.tasks.summarize_and_store_evicted_turns': {'queue': 'default'},
    }
)
...
def _process_chunks(self, chunks, metadata):
    ...
    and store them in Milvus. Processes chunks in batches to be memory-safe.
    """
    import itertools
    import polars as pl
    from datetime import datetime, timezone
    
    # Check if iterable is empty by trying to get first element
    chunks_iterator = iter(chunks)
    try:
        first_el = next(chunks_iterator)
        # Restore first element to the stream
        chunks_iterator = itertools.chain([first_el], chunks_iterator)
    except StopIteration:
        logger.warning("Received empty chunks list/iterator. Skipping ingestion commit.")
    ...
    # 1. Just-in-Time (JIT) Purge: delete old chunks and graph facts right before inserting the new ones.
    # Done once at the start to avoid blackouts.
    ...
    while True:
        sub_batch = list(itertools.islice(chunks_iterator, sub_batch_size))
        if not sub_batch:
            break
            
        sub_batch_len = len(sub_batch)
        batch_start = processed_count
        batch_end = processed_count + sub_batch_len
        processed_count = batch_end
        
        self.update_state(state='PROGRESS', meta={
            'current': batch_start, 
            'total': -1, # Total size is unknown for generators
            'status': f'Generating Embeddings & AI Questions ({batch_start}-{batch_end})...'
        })
        ...
        sub_batch_dense = [r[0].tolist() for r in hybrid_results]
        sub_batch_sparse = [r[1] for r in hybrid_results]
        ...
        sub_enriched_metadata_list = []
        sub_final_child_chunks = []
        for j, q_list in enumerate(questions_batch):
            ...
            for s, p, o in triplets_batch[j]:
                graph_manager.add_relationship(s, p, o, {"source": metadata.get("source"), "chunk_idx": batch_start+j})
            ...
            if is_hierarchical:
                ...
                sub_final_child_chunks.append(child_text)
            else:
                sub_final_child_chunks.append(sub_batch[j])
                
            sub_enriched_metadata_list.append(chunk_meta)

        # Prepare data for high-speed insertion into Milvus Lite
        df_meta = pl.DataFrame(sub_enriched_metadata_list)
        ...
        for j in range(len(clean_metadatas)):
            ...
            if is_visual:
                ...
                record = {
                    "embedding": v_emb,
                    "content": sub_final_child_chunks[j],
                    **meta
                }
                ...
                record = {
                    "embedding": sub_batch_dense[j],
                    "sparse_vector": sub_batch_sparse[j],
                    "content": sub_final_child_chunks[j],
                    **meta
                }
        ...
        logger.info(f"Progress: Ingested batch of {sub_batch_len} chunks (Total: {processed_count}).")

    logger.info(f"Task SUCCESS: Processed {processed_count} chunks using {model_name}.")
    return {"status": "success", "count": processed_count, "model": model_name}
...
def process_file_ingestion(self, file_path, heavy_parsing, metadata):
    ...
    from app.services.loaders import extract_chunks_stream
    
    try:
        # 1. Perform document loading and structural chunking (fully async/background)
        chunks = extract_chunks_stream(file_path, heavy_parsing, hierarchical=True)
        logger.info(f"Started streaming chunks from {file_path}.")
...
def summarize_and_store_evicted_turns(self, session_id: str, evicted_messages: list):
    """
    Background task to summarize evicted conversation turns and store the summary
    as an episodic/semantic memory in Milvus, preventing short-term memory loss.
    """
    logger.info(f"Summarizing {len(evicted_messages)} evicted messages for session: {session_id}")
    if not evicted_messages:
        return {"status": "skipped", "reason": "no_messages"}

    # Format the dialogue
    dialogue = "\n".join([f"{msg['role'].upper()}: {msg['content']}" for msg in evicted_messages])
    
    # Prompt the ingestion model to summarize
    prompt = (
        "You are an Episodic Memory Compactor. Below is a portion of a conversation that is being evicted from the short-term window.\n"
        "Extract all key facts, commitments, configurations, and context mentioned in this dialogue into a concise, high-fidelity summary.\n"
        "Focus on exact numbers, technical IDs, and specific actions. Avoid generic statements.\n\n"
        f"DIALOGUE:\n{dialogue}\n\n"
        "EPISODIC SUMMARY:"
    )
    try:
        # Generate summary using the high-throughput ingestion model
        if ai_manager.llm_ingest:
            response = ai_manager.llm_ingest.invoke(prompt)
            summary_text = response.content.strip()
        else:
            return {"status": "failed", "reason": "no_ingest_llm"}
            
        # Store as episodic memory in Milvus
        metadata = {
            "source": f"session_{session_id}_eviction",
            "filename": f"session_{session_id}",
            "type": "chat_log",
            "skip_enrichment": True
        }
        
        # Trigger standard batch store
        result = _process_chunks(self, [f"[EPISODIC_MEMORY_SUMMARY]: {summary_text}"], metadata)
        logger.info(f"Successfully stored episodic summary for session {session_id}")
        return {"status": "success", "summary": summary_text}
    except Exception as exc:
        logger.error(f"Failed to summarize evicted memory: {exc}")
        raise self.retry(exc=exc)
